...
int main(int argc, const char * argv[])
{
    ...
    for (n = 0; n < 9 && getintfix(&arr[n]) != EOF; n++)
        ;
    for (n = 0; n < 9; n++)
    {
        printf("%d\n", arr[n]);
...
int getintfix(int *pn)//getint接受自由格式的输入并执行转换,将输入的字符流分解成整数,且每次调用得到一个整数,getint返回转换后得到的整数,并在到达输入结尾时返回EOF
//将标识是否到达文件末尾的状态作为getint函数的返回值,同时使用一个指针参数存储转换后的整数并传回给主调函数,函数scanf实现就采用这种方式
//上一个例子中,如果符号+或者-的后面紧跟着的不是一个数字,getint函数将把符号视为数字0的有效表达方式,这一版本修改原来的getint函数机制
//将这种形式的+或者-符号重新写回输入流中
{
    int c, d, sign;
    ...
    {
        d = c;//先保存符号到d
        if (!isdigit(c = getch()))//如果下一个字符既不是一个数字也不是一个EOF标记,就把新读入的这个字符压回输入缓冲区,然后getintfix函数会将d中保存的符号压回输入缓冲区并返回符号说明情况
        {
            if (c != EOF)
            {
                ungetchs(c);
...
//上一个例子中,如果符号+或者-的后面紧跟着的不是一个数字,getint函数将把符号视为数字0的有效表达方式,这一版本修改原来的getint函数机制
//将这种形式的+或者-符号重新写回输入流中,新函数为getintfix
